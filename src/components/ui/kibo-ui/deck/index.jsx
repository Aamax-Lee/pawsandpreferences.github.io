...
const DeckCard = ({ children, onSwipe, threshold, style, exitDirection }) => {
  ...
  const heartOpacity = useTransform(x, [-150, 0], [1, 0]);
  const crossOpacity = useTransform(x, [150, 0], [1, 0]);
...
export const DeckItem = ({ className, ...props }) => (
  <div
    className={cn(
      "flex h-full w-full items-center justify-center rounded-lg  bg-card text-card-foreground shadow-lg",
      className
    )}
    {...props}
  />
...
export const DeckEmpty = ({ children, className, ...props }) => (
  <div
    className={cn(
      "absolute inset-0 flex items-center justify-center rounded-lg  text-muted-foreground",
      className
    )}
    {...props}
  >
